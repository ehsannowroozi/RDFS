...
if __name__ =='__main__':
    ...
    LabledTrainPath=r'.........................................txt'
    LabledValPath=r'.........................................txt'
    LabledTestPath=r'......................................txt'
    ...
    adversarial_evaluations_dict={'StammNet_model_fe_BIM0.3_Limit40.npy':np.array([0, 0], dtype='float64'),
                             'StammNet_model_fe_BIM0.3_Limit50.npy':np.array([0,0], dtype='float64'),
                                  'StammNet_model_fe_BIM0.3_Limit55.npy': np.array([0, 0], dtype='float64'),
                             'StammNet_model_fe_LBFGS_Limit40.npy':np.array([0,0], dtype='float64'),
    ...
    if PATTERN_FOLDER is None:
        for runtime in range(RUN_TIMES):
            ...
            #     NUM_FEATURES, PRISTINE_training, MANIPULATE_training, PRISTINE_validation, MANIPULATE_validation,PRISTINE_testing,
            ...
            ''' start training'''
            early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.0005, patience=EARLY_STOP_PATIENCE, restore_best_weights=True)
            ...
            history = top_fc_layers.fit_generator(prepare_random_data_generator(BATCH_SIZE,LabledTrainPath,pattern),
                                                  steps_per_epoch=int(20000/BATCH_SIZE),
                                                  epochs=EPOCHS,
                                                  callbacks=[early_stopping],
                                                  validation_data=prepare_random_data_generator(BATCH_SIZE,LabledValPath,pattern),
            ...
            evaluation = top_fc_layers.evaluate_generator(prepare_random_data_generator(BATCH_SIZE,LabledTestPath,pattern),
        ...
        for runtime, patternfile in enumerate(os.listdir(PATTERN_FOLDER)):
            ...
            # training_set, training_labels, validation_set, validation_labels, testing_set, testing_labels = prepare_data_function(
            #     NUM_FEATURES,PRISTINE_training,MANIPULATE_training,PRISTINE_validation,MANIPULATE_validation,PRISTINE_testing,MANIPULATE_testing, RANDOM_SEED,FULL_FEATURE_SIZE, pattern)
            top_fc_layers = build_top_layer(NUM_FEATURES_USED, HIDDEN_NODES)
            ''' start training'''
            early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.5, patience=EARLY_STOP_PATIENCE, restore_best_weights=True)
            ...
            history = top_fc_layers.fit_generator(prepare_random_data_generator(BATCH_SIZE, LabledTrainPath, pattern),
                                                  ...
                                                  epochs=EPOCHS,
                                                  callbacks=[early_stopping],
                                                  validation_data=prepare_random_data_generator(BATCH_SIZE, LabledValPath,
                                                                                                pattern),
            ...
            evaluation = top_fc_layers.evaluate_generator(
                prepare_random_data_generator(BATCH_SIZE, LabledTestPath, pattern),
